...
const url = new URL(window.location.href);
const room = url.searchParams.get("room");
// let my_camera = document.createElement("video")
// my_camera.muted = true;
// my_camera.dataset.id = '1'
const peer = new Peer()
const peers = {}

peer.on("open", id => {
  my_camera = document.createElement("video")
  my_camera.muted = true;
  my_camera.dataset.id = id

  navigator.mediaDevices.getUserMedia({ video: true, audio: true }).then(stream => {
    socket.emit("join-room", { room, id });
    addNewVideo(my_camera, stream);

    peer.on("call", call => {
      const my_stream = document.createElement("video");
      my_stream.dataset.id = call.peer;
      call.answer(stream);
      call.on("stream", friend_stream => {
        addNewVideo(my_stream, friend_stream);
      })
    })
    socket.on("user-connected", userId => {
      console.log(userId);
      aNewConection(userId, stream);
    })
  })
  ...
  const aNewConection = (userId, stream) => {
    const call = peer.call(userId, stream);
    const friend_camera = document.createElement("video");
    friend_camera.dataset.id = userId;
    call.on("stream", friend_stream => {
      addNewVideo(friend_camera, friend_stream);
    })

    call.on('close', () => {
      friend_camera.remove()
    })
    peers[userId] = call
  }
